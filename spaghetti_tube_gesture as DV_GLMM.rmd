...
load(file ="GLMM_S03_Gesture_workspace.rData")
...
mm1Gesture_int <- glmer(Gesture ~ NonsocialRoom3+SpaghettiTube*NonsocialRoom4 + Species+ z.Session+z.Trial+
...
## code to run the model
mm1Gesture.null_int <- glmer(Gesture ~ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
```{r}	 
anova(mm1Gesture_int, mm1Gesture.null_int, test="Chisq")
```
```{r}
round(summary(mm1Gesture_int)$coefficients, 3)
```

```{r}
xdrop1gesture_int=drop1(mm1Gesture_int, test="Chisq")
round(xdrop1gesture_int,3)
```
...
## code to run the model
mm1Gesture <- glmer(Gesture ~ NonsocialRoom3+SpaghettiTube+NonsocialRoom4 + Species+ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
## code to run the model
mm1Gesture.null <- glmer(Gesture ~ 1 +
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
####Full-null model comparison
```{r}	 
anova(mm1Gesture, mm1Gesture.null, test="Chisq")
```
####Model output
  + Coefficients
```{r}
round(summary(mm1Gesture)$coefficients, 3)
```
...
```{r}
xdrop1gesture=drop1(mm1Gesture, test="Chisq")
round(xdrop1gesture,3)
```
...
```{r}
save.image("GLMM_S03_Gesture_workspace.rData")
