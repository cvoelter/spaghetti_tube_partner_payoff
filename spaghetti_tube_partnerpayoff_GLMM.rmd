...
####Null model	 
```{r}	 
mm.2.null <- glmer(tool_trans~ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
####Null model	 
```{r}	 
mm.3.null <- glmer(tool_trans~z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
```{r}
tab_model(mm5, show.est = TRUE, show.se = TRUE, show.stat = TRUE, transform = NULL, show.ci = FALSE,file="mm5_output_tab.doc")
...
#GLMM 5 with Gesture and Gesture*Species interaction: Species relevelled
...
## code to run the model
mm5_Species2 <- glmer(tool_trans~NonsocialRoom3 + SpaghettiTube +NonsocialRoom4  + Presence+ Gesture*Species2+ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
