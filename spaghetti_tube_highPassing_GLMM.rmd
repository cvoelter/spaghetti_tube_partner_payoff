...
load(file ="GLMM_S02_HighPassing_workspace.rData")
...
mm1High <- glmer(Highpass~SpaghettiTube+NonsocialRoom4  + Presence+ Gesture*Species+ z.Session+z.Trial+
...
```{r}	 
anova(mm1High, mm1High.null, test="Chisq")
```
####Model output
  + Coefficients
```{r}
round(summary(mm1High)$coefficients, 3)
```
...
```{r}
xdrop1high=drop1(mm1High, test="Chisq")
round(xdrop1high,3)
```
...
## code to run the model
mm2High <- glmer(Highpass~SpaghettiTube+NonsocialRoom4  + Species+ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
## code to run the model
mm2High.null <- glmer(Highpass~1+ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
####Full-null model comparison
```{r}	 
anova(mm2High, mm2High.null, test="Chisq")
```
...
####Model output
  + Coefficients
```{r}
round(summary(mm2High)$coefficients, 3)
```
...
Drop1: P values for the individual effects were based on likelihood ratio tests comparing the full with respective reduced models (Barr et al., 2013; R function drop1 with argument 'test' set to "Chisq"). 
```{r}
xdropHigh2=drop1(mm2High, test="Chisq")
round(xdropHigh2,6)
```
...
```{r}
save.image("GLMM_S02_HighPassing_workspace.rData")
