...
load(file ="ToolRecipient_models_SpaghettiTube_partnerPayoffs.RData")
...
## code to run the model
library(brms)
bmm1 <- brm(tool_intube~ SpaghettiTube*NonsocialRoom4  + Highpass +
             Species+ z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
...
#plot(bmm1)
#marginal_effects(bmm1)


bmm1_no.int <- update(bmm1, formula = ~ .-SpaghettiTube:NonsocialRoom4)


BF_brms_savage = hypothesis(bmm1 , hypothesis = 'SpaghettiTubeNothing:NonsocialRoom4Low = 0')  # H0: No interaction
BF_brms_savage$hypothesis

BF_brms = bayes_factor(bmm1, bmm1_no.int)
BF_brms

#Estimated Bayes factor in favor of bridge1 over bridge2: 141.07699
...
save.image("ToolRecipient_models_SpaghettiTube_partnerPayoffs.RData")
```
...
####Null model	 
```{r}	 
mm.1.null <- glmer(tool_intube~z.Session+z.Trial+
             (1|Room3)+(1|Room4)+ (1|Dyad)+
